...
(function () {
  ...
  /* A refresh always starts from the beginning of the document. */
  ...
  function resetInitialScroll() {
    ...
    window.scrollTo(0, 0);
    if (previousBehavior) rootStyle.scrollBehavior = previousBehavior;
  ...
  function initTabs(rootSelector, buttonSelector, panelSelector, idPrefix) {
    qsa(rootSelector).forEach(function (root, rootIndex) {
      ...
      function select(selectedButton, animate) {
        ...
        panels.forEach(function (panel) {
          var isActive = panel.getAttribute("data-panel") === selectedKey;
          panel.hidden = !isActive;
          if (isActive && animate && window.SenergyMotion) window.SenergyMotion.panel(panel);
        });
      }
  ...
  function initRegistration() {
    ...
    form.addEventListener("submit", function (event) {
      ...
      var message = [
        ...
        "Телефон: " + phone,
        "",
        "Принимаю публичную оферту. Даю ИП SENERGY (ИИН/БИН 821230350645) согласие на обработку ФИО и телефона только для записи на марафон и связи по этой заявке, включая передачу в WhatsApp/Meta и трансграничную обработку. Согласие действует до завершения обработки заявки или отзыва, с учётом обязательного хранения по закону. Условия — в политике конфиденциальности.",
        "Документы: https://senergies.kz/offer.html и https://senergies.kz/privacy.html",
        "Политика и согласие: версия " + form.dataset.consentVersion,
        "Оферта: редакция " + form.dataset.offerVersion,
